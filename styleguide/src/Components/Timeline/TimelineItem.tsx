...
export const TimelineItem = ({
  ...
  const iconBackgroundColor = item.iconBackgroundColor || `bg-inverted-1`
  ...
  return (
    <li className={`timeline-item relative ${!lastItem ? 'mb-10' : ''}`}>
      ...
      <div className="ml-7 pt-px">
        ...
        >
          {item.description && (
            <button className={`float-right p-1 -mt-px text-inverted-2 group-hover:text-inverted-1`}>
              <Icon icon="angleLeft" size={4} className={`-mt-px transition-transform ${isOpen ? 'rotate-90' : '-rotate-90'}`} block />
            </button>
          )}
          <span>
            {item.title}
        ...
        {item.description && (
          <div
            className={`timeline-description overflow-hidden text-sm text-inverted-1 break-words transition-height ${isOpen ? 'max-h-96' : 'max-h-0'}`}
          >
            {item.description}
          </div>
        )}
