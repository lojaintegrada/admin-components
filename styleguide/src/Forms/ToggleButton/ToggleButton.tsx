...
const ToggleButtonComponent =(
  ...
  const toggleItems =  children.map( (item, index) => { 
      const borderRadius = (index === 0? 'rounded-l':(( children.length - 1) === index ? 'border-r rounded-r':'') );
      return (
        <li key={`toggle-btn-[${index}]`} className='inline-flex justify-center items-center h-12'> 
          <label htmlFor={`toggle-btn-${index}`} className='relative inline-flex justify-center items-center h-12'>
            <input  
              ref={ref}
              ...
              {...props}
              />
            <div className={`checkmark ${toggleClasses} text-f6 font-bold inline-flex justify-center items-center ${borderRadius} py-4 px-6 border-y border-l h-12`} key={`toggle-btn-[${index}]`}>
              {item}
            </div> 
        </label> 
      </li>
  ...
  return (
      <div className={`inline-flex justify-center items-center h-12 ${toggleClasses}} rounded`}>
...
export interface ToggleButtonProps extends React.InputHTMLAttributes<HTMLInputElement>{
  children: React.ReactNode[] | ((props: ToggleButtonProps) => React.ReactNode)[]
}
