...
const InformationBoxComponent = ({
  ...
  subtitle,
    ...
    return (
      ...
      >
        <div className={`w-6 h-6 flex justify-center items-center flex-shrink-0 rounded-full mr-3 bg-base-1`}>
          <Icon
        ...
        <div className="flex-grow flex flex-col sm:flex-row items-start items-center justify-between min-w-0">
          <div className="flex flex-col justify-center min-w-0 break-words tracking-4 leading-7 text-on-base">
            <span className={`InformationBox-title text-f5 font-bold ${InformationBoxTypes[type].iconClass}`}>{InformationBoxTypes[type].title}</span>
            {subtitle && <span className="InformationBox-subtitle text-f6 font-regular mt-1">{subtitle}</span>}
